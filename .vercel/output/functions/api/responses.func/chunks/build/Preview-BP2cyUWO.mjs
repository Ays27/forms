import { ref, defineComponent, unref, computed, createVNode, resolveDynamicComponent, mergeProps, useSSRContext } from 'vue';
import { ssrInterpolate, ssrRenderList, ssrRenderComponent, ssrRenderVNode, ssrRenderAttrs, ssrRenderAttr } from 'vue/server-renderer';
...
const useFormStore = pinia_prodExports.defineStore("form", () => {
  ...
  async function setPublished(state) {
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[--><div class="flex items-center gap-3 mb-6 px-2"><button class="p-2 rounded hover:bg-gray-100"> ← </button></div><div class="max-w-3xl mx-auto py-8 space-y-6"><div class="bg-white p-6 rounded-xl border-t-4 border-orange-600"><h1 class="text-2xl font-bold">${ssrInterpolate(unref(formTitle))}</h1><p class="text-gray-600">${ssrInterpolate(unref(formDescription))}</p></div><!--[-->`);
      ssrRenderList(unref(questions), (q) => {
        _push(`<div class="bg-white p-6 rounded-xl space-y-4"><h2 class="font-medium text-lg">${q.title ?? ""}</h2>`);
...
//# sourceMappingURL=Preview-BP2cyUWO.mjs.map
